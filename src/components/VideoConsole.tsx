...
export function VideoConsole({ isConnected }: VideoConsoleProps) {
  ...
  return (
    <Card className="relative flex-1 bg-gradient-card border-border/50 shadow-card overflow-hidden">
      ...
      <div className="aspect-video bg-muted/50 flex items-center justify-center relative border border-border/30">
        ...
        {isConnected && isCamEnabled ? (
          <div className="relative w-full h-full">
            <PipecatClientVideo 
              ...
              onResize={({ aspectRatio, height, width }) => {
                console.log("Local video dimensions:", { aspectRatio, height, width });
              }}
            />
          ...
          <div className="flex flex-col items-center gap-4 text-muted-foreground">
            {!isConnected ? (
              <>
                <Video size={64} />
                <p className="text-lg">Connect to start video</p>
              </>
            ) : (
              <>
                <VideoOff size={64} />
        ...
        {isConnected && (
          <div className="absolute top-4 right-4 flex items-center gap-2 bg-success/20 backdrop-blur-sm px-3 py-1 rounded-full border border-success/30">
      ...
      <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex items-center gap-2 bg-background/80 backdrop-blur-sm px-4 py-2 rounded-full border border-border/50">
        ...
        <div className="relative group">
          ...
          {isConnected && (
            <div className="absolute bottom-full mb-2 left-1/2 transform -translate-x-1/2 bg-background border border-border rounded-lg p-3 shadow-lg opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none group-hover:pointer-events-auto min-w-48">
              ...
              {availableCams.length === 0 && availableMics.length === 0 && (
      ...
      {isConnected && (
        <div className="absolute top-2 left-2 flex gap-1">
          <div className={`w-2 h-2 rounded-full ${isMicEnabled ? 'bg-success' : 'bg-destructive'}`} 
               title={`Microphone ${isMicEnabled ? 'enabled' : 'disabled'}`} />
          <div className={`w-2 h-2 rounded-full ${isCamEnabled ? 'bg-success' : 'bg-destructive'}`}
               title={`Camera ${isCamEnabled ? 'enabled' : 'disabled'}`} />
        </div>
      )}
    </Card>
  );
